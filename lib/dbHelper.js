...
module.exports = (knex) => {
  return{
    getVoteCountByPollId: (polls_id)=>{
      return knex('voters').count().where({ polls_id});
    },
    checkAdminCode: (adminCode) => {
      return knex.select().from('polls').where({ admin_code: adminCode}).limit(1);
    },
    getPollIdAdminCode: (adminCode) => {
      return knex.select('id').from('polls').where({ admin_code: adminCode}).limit(1);
    },
    checkSubCode: (subCode) => {
      return knex.select().from('polls').where({ submission_code: subCode}).limit(1);
    },
    ...
    getPollAndChoicesBySubCode: (submissionCode) => {
      return knex.select('polls.question', 'polls.creator_name','polls.admin_code','polls.creator_email', 'polls.anonymous', 'choices.id', 'choices.description', 'choices.title')
    ...
    getSubCodeByAdminCode: (admin_code)=>{
      return knex('polls').select('submission_code').where({admin_code});
    }
  }
}
